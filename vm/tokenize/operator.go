...
const pattern = `(?i)(?P<or>\|\|)|(?P<and>&&)|(?P<not>!)|(?P<equal>=)|(?P<du_equal>==)|(?P<not_equal>!=)|(?P<lt_equal><=)|(?P<gt_equal>>=)|(?P<lt><)|(?P<gt>>)|(?P<plus>\+)|(?P<minus>-)|(?P<multi>\*)|(?P<square>\*\*)|(?P<divide>/)|(?P<remain>%)|(?P<lparen>\()|(?P<rparen>\))|(?P<lpfunc>\{)|(?P<rpfunc>\})|(?P<comma>,)|(?P<colon>:)|(?P<var>\bvar\b)|(?P<val>\bval\b)|(?P<const>\bconst\b)|(?P<else_if>\belse if\b)|(?P<else>\belse\b)|(?P<if>\bif\b)|(?P<func>\bfunc\b)|(?P<fors>\bfor\b)|(?P<whiles>\bwhile\b)|(?P<break>\bbreak\b)|(?P<continue>\bcontinue\b)|(?P<return>\breturn\b)|(?P<print>\bprint\b)|(?P<printf>\bprintf\b)|(?P<println>\bprintln\b)|(?P<import>\bimport\b)|(?P<call>\bcall\b)|(?P<float>[-\d_]+(?:\.\d+)?)|(?P<number>[-\d_]+)|(?P<string>"[^"]*")|(?P<olist>\[)|(?P<clist>\])`
	...
	for idx, match := range matches {
		...
		for i, name := range re.SubexpNames() {
			if i > 0 && match[i] != "" {
				...
				case "colon":
					token = TOKEN{Type: COLON, Literal: match[i], StartIndex: startIndex, EndIndex: endIndex}
				case "var":
